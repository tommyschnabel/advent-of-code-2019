...
func part2() {
	input1, input2 := getIntpus()
	intersec, err := smallestNumberOfStepsToIntersection(input1, input2)
	if err != nil {
		panic(err)
	}
...
func part1() {
	input1, input2 := getIntpus()
	distance, err := distanceFromStartToClosestIntersection(input2, input1)
	if err != nil {
		panic(err)
	}
...
func distanceFromStartToClosestIntersection(input1, input2 string) (int, error) {
	intersection, err := findClosestIntersection(input1, input2)
	if err != nil {
		return 0, err
	}
	...
	return getDistanceFromStart(intersection.coord), nil
}

func smallestNumberOfStepsToIntersection(input1, input2 string) (*intersection, error) {
	intersections, err := findIntersections(input1, input2)
	if err != nil {
		return nil, err
	}

	var smallestSteps int
	var smallestStepIntersection intersection
	for _, intersec := range intersections {
		if smallestSteps == 0 || intersec.steps < smallestSteps {
			smallestSteps = intersec.steps
			smallestStepIntersection = intersec
		}
	}

	return &smallestStepIntersection, nil
}

func getDistanceFromStart(c coord) int {
	x := c.x
	if x < 0 {
		x = x * -1
	}
...
func findClosestIntersection(input1, input2 string) (*intersection, error) {
	...
	if len(intersections) == 0 {
		return nil, errors.New("found no intersections")
	}

	for _, c := range intersections {
		dist := getDistanceFromStart(c.coord)
	...
	if closestDistance == 0 {
		return nil, errors.New("closest intersection calculated")
	}

	return &closestIntersection, nil
}

func findIntersections(input1, input2 string) ([]intersection, error) {
	line1, err := getLinePath(input1)
	if err != nil {
		return nil, err
	}

	line2, err := getLinePath(input2)
	if err != nil {
		return nil, err
	}
...
func visit(c coord, visitedPoints map[int]map[int]int, steps int) (visited bool, totalSteps int) {
	ys, ok := visitedPoints[c.x]
	if !ok {
		ys = make(map[int]int)
		visitedPoints[c.x] = ys
	}

	if steps, ok := ys[c.y]; ok && steps > 0 {
		visited = true
	}
...
func getLinePath(input string) (line, error) {
	directions := strings.Split(input, inputSeparator)
	line := line{}

	var x, y int

	for _, direction := range directions {
		length, err := strconv.Atoi(direction[1:])
		if err != nil {
			return nil, err
		}
		...
		default:
			return nil, fmt.Errorf("wtf is this direction: %c", direction[0])
		}
	}
